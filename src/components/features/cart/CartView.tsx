...
/**
 * CartView.tsx — 장바구니 뷰
 * useShipping 훅 + EmptyState 컴포넌트 적용
 */
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        {items.map(({ product, quantity, selectedSize }) => (
          <div key={`${product.id}-${selectedSize}`}
            className="flex gap-4 p-5 bg-white rounded-2xl border border-gray-100 hover:border-gray-200 transition-colors">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  {selectedSize && (
                    <span className="text-xs text-gray-400">사이즈: {selectedSize}</span>
                  )}
                </div>
                <button
